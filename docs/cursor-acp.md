...
| Probe host | Linux x86_64 (dev machine, not the target Core host) |
| Probe date | 2026-08-23 |

The probe scripts live in `agent-core/tools/acp_probe/` so the findings can be re-verified on the
Core host before deployment. Re-run them there: the CLI is the same JS bundle on both platforms,
...
on the Core host before deploying:
