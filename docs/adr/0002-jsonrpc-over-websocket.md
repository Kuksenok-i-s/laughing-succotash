...
The Core host sits behind a residential NAT with no stable public address. Both sides need to
...
- **REST + callback URL** — requires an inbound listener on the Core host, which is the thing we
...
No inbound exposure on the Core host. The Core owns reconnection with exponential backoff and full
