"""Trainer-journal skill text, seeded into each Telegram user's Cursor workspace."""

from __future__ import annotations

from pathlib import Path

SKILL_NAME = "trainer-journal"
...
def seed_into(workspace: Path) -> Path:
    """Write the skill where Cursor ACP can discover it (the per-user sandbox cwd)."""
    target = workspace / ".cursor" / "skills" / SKILL_NAME / "SKILL.md"
    target.parent.mkdir(parents=True, exist_ok=True)
    if not target.is_file() or target.read_text(encoding="utf-8") != SKILL_MARKDOWN:
        target.write_text(SKILL_MARKDOWN, encoding="utf-8")
    return target
