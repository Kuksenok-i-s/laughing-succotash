...
Вечерний дневник по расписанию — только по желанию. Начните новый разговор и напишите:
Включи дневник для меня
Старые записи сохранены. /journal открывает сегодняшний день и без расписания.
...
def build_router(bot: Bot, store: GatewayStore, core, submissions, settings) -> Router:
    ...
    def authorized(user_id: int | None) -> bool:
        ...
        avoids waking the Core host for strangers — never the security boundary.
        """
