...
Run as root on the Core host. Does not install dependencies, run migrations or replace config/secrets.
...
Paths come from the environment (see docs/infra.local.md):
CORE_INSTALL_ROOT, CORE_RELEASES_DIR, CORE_DATABASE.
"""
import argparse
...
def main():
    ...
    backup = env_path('CORE_RELEASES_DIR') / datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')
    apply(env_path('CORE_INSTALL_ROOT'), args.stage, backup, manifest,
          lambda: run('systemctl', 'stop', 'agent-core'),
          lambda: run('systemctl', 'start', 'agent-core'), health,
          lambda: not busy(env_path('CORE_DATABASE')))
