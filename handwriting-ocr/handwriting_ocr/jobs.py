...
gone and fails the OCR job. What is on disk is only the image, and only until the result has been
collected.
"""
...
class JobStore:
    ...
    def submit(self, job: Job) -> Job:
        ...
        log.info("job %s queued (%s)", job.job_id, job.filename)
        return job
    ...
    def report(self, job_id: str, *, percent: float, stage: str) -> None:
        with self._lock:
            job = self._jobs.get(job_id)
